...
export default function ClientPortal() {
  ...
  const pct = documents.length > 0
    ? Math.round(received / documents.length * 100)
    : 0
      ...
      <div className="max-w-2xl mx-auto p-6">
        ...
        {overdue > 0 && (
          <div className="mt-4 bg-red-50 border border-red-200 rounded-xl p-4">
            <p className="text-sm text-red-700 font-medium">⚠️ You have {overdue} overdue document{overdue > 1 ? 's' : ''}</p>
            <p className="text-xs text-red-600 mt-1">Please upload them immediately to avoid filing delays.</p>
          </div>
        )}
      </div>
    </main>
